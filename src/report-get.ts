...
import { apaMatchToMatch, apaMatchToPlayerMatches, apaTeamToTeam, Match, PlayerMatch, PlayerReport, TeamSeason } from "./types";

export function generateReport(
  teams: TeamSeason[], 
  matchesByTeam: Record<string, Array<Match & {playerMatches: Array<PlayerMatch>}>>,
  ...
  for (const team of teams) {
    ...
    for (const match of teamMatches) {
      // Calculate team situation up to each player match
      let homeTeamScoreUpToNow = 0;
      let awayTeamScoreUpToNow = 0;
      
      for (const playerMatch of match.playerMatches) {
        const isPlayerHome = playerMatch.homePlayerStats.id === playerIdByTeam[team.id];
        const isPlayerAway = playerMatch.awayPlayerStats.id === playerIdByTeam[team.id];

        if (!isPlayerHome && !isPlayerAway) {
          // Update running scores for non-player matches
          const homePlayerWon = playerMatch.homePlayerStats.score > playerMatch.awayPlayerStats.score;
          if (homePlayerWon) {
        ...
        const playerStats = isPlayerHome ? playerMatch.homePlayerStats : playerMatch.awayPlayerStats;
        const opponentStats = isPlayerHome ? playerMatch.awayPlayerStats : playerMatch.homePlayerStats;
        totalMatches++;
        ...
        // Track by team situation (winning/tied/losing) up to this player's match
        let teamSituation = "team_tied";
        if (isPlayerHome) {
          if (homeTeamScoreUpToNow > awayTeamScoreUpToNow) teamSituation = "team_winning";
          else if (homeTeamScoreUpToNow < awayTeamScoreUpToNow) teamSituation = "team_losing";
        } else {
          if (awayTeamScoreUpToNow > homeTeamScoreUpToNow) teamSituation = "team_winning";
          else if (awayTeamScoreUpToNow < homeTeamScoreUpToNow) teamSituation = "team_losing";
        }
        ...
        // Update running scores after this player's match
        if (isWin) {
...
export const handleReportGet = async (memberId: string, apaClient: APAClient): Promise<any> => {
  ...
  for (const t of apaTeams) {
    const normalizedTeamId = teams.find(team => team.apaId === t.team.id.toString())?.id;
    if (!normalizedTeamId) {
      console.log(`No normalized team id found for team ${t.team.id}`);
      continue;
    }

    playerIdByTeam[normalizedTeamId] = t.id;
  }
  console.log(playerIdByTeam);
  
  const matchesByTeam: Record<string, Array<Match & {playerMatches: Array<PlayerMatch>}>> = {};
  
  for (const team of teams) {
    console.log(`Processing team: ${team.name} (ID: ${team.id}, Session: ${team.season} ${team.seasonYear})`);
    ...
    for (const teamMatch of teamMatches) {
      try {
        ...
        console.log(`  Could not get match details for match ${teamMatch.id}:`, error);
      }
    }
  }
